...
class HirerCvRequestTest extends TestCase
{
    ...
    public function testParamValidation($params)
    {
        $this->actingAs($this->hirer, 'hirers')
            ->json('PATCH', route('hirer.search.results.request', $this->search->id), $params)
    ...
    {
        $this->actingAs($this->hirer, 'hirers')
            ->json('PATCH', route('hirer.search.results.request', 'not-a-id'), [
    ...
    public function testHirerCantEditSearchesThatDontBelongToTheirLawFirm()
    {
        ...
        $this->actingAs($this->hirer, 'hirers')
            ->json('PATCH', route('hirer.search.results.request', $search->id), [
                'candidate_id_list' => [$this->candidate->id],
            ])
    ...
    public function testCandidatesAreBlocked()
    {
        ...
        $this->actingAs($candidate, 'candidates')
            ->json('PATCH', route('hirer.search.results.request', $this->search->id), [
                'candidate_id_list' => [$this->candidate->id],
            ])
            ->assertResponseStatus(401);
    }

    public function testHirersAreBlockedFromEditingSearchesThatDontHaveAStatusOfZero()
    {
        $search = factory(Search::class)->create(['hirer_id' => $this->hirer->id]);
        $search->matches()->sync([
            $this->candidate->id => [
                'status' => 200
            ]
