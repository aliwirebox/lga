...
@section('content')
    ...
    <div class="row">
        @if ($blogs->isEmpty())
            <div class="well text-center">No blog posts found.</div>
        @else
            <table class="table table-striped">
                ...
                @foreach($blogs as $blog)
                    <tr>
                        <td>
                            <a href="{!! route('quarx.blogs.edit', [$blog->id]) !!}">{!! $blog->title !!}</a>
                        </td>
                        <td class="text-right">
                            <form method="post" action="{!! route('quarx.blogs.destroy', [$blog->id]) !!}">
                                {!! csrf_field() !!}
                                {!! method_field('DELETE') !!}
                                <button class="delete-btn btn btn-xs btn-danger pull-right" type="submit"><i class="fa fa-trash"></i> Delete</button>
                            </form>
                            <a class="btn btn-xs btn-default raw-margin-right-8 pull-right" href="{!! route('quarx.blogs.edit', [$blog->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
